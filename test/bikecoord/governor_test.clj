(ns bikecoord.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [bikecoord.store :as store]
            [bikecoord.advisor :as advisor]
            [bikecoord.governor :as governor]))
...
(def ^:private req {:repairer-id "repairer-1"})
...
(deftest hard-on-unregistered-repairer
  (let [st (fresh-store)
        v (governor/check {:repairer-id "nobody"} {} (op :log-work-record) st)]
    (is (:hard? v))
    (is (some #(= :no-repairer (:rule %)) (:violations v)))))
...
(deftest hard-on-scope-excluded-op-declare-bicycle-roadworthy
  (testing "declaring a bicycle roadworthy is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :declare-bicycle-roadworthy) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-clear-bicycle-for-sale
  (testing "clearing a bicycle for sale is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :clear-bicycle-for-sale) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-finalize-repair-decision
  (testing "finalizing a repair-execution decision is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :finalize-repair-decision) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest hard-on-scope-excluded-rationale-even-with-allowlisted-op
  (testing "defense-in-depth: a rationale that itself attempts to finalize the repair is blocked even if the op looks routine"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :schedule-crew-operation)
                                           :rationale "recommend we finalize the repair now")
                             st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-rationale-declare-roadworthy-and-cleared-for-sale
  (testing "defense-in-depth: a rationale that attempts to declare the bicycle roadworthy and cleared for sale is blocked even if the op looks routine"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :log-work-record)
                                           :rationale "recommend we declare the bicycle roadworthy and cleared for sale")
                             st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest always-escalates-safety-concern-even-at-high-confidence
  (testing "a mechanical-hazard/equipment-condition concern always requires human sign-off"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :flag-safety-concern :hazard-type :brake-failure-risk)
                                           :confidence 0.99)
                             st)]
      (is (not (:hard? v)))
      (is (:escalate? v)))))
...
(deftest default-mock-advisor-proposals-never-self-trip-on-scope-exclusion
  (testing "the governor's scope-exclusion term list must never match the mock advisor's own default rationale text for any allowlisted op — CLAUDE.md's known self-tripping bug pattern (rationale legitimately contains bare nouns like 'repair'/'bicycle'/'shop'/'safety officer', but never the full finalization-action phrases)"
    (let [st (fresh-store)
          adv (advisor/mock-advisor)
          ops [:log-work-record :schedule-crew-operation
               :flag-safety-concern :coordinate-supply-order]]
      (doseq [o ops]
        (let [request {:repairer-id "repairer-1" :op o :shop-id "S-1"
                        :stake :low :task "routine repair-bay task"
                        :hazard-type :brake-failure-risk :cost 500}
