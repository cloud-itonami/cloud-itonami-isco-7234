(ns bikecoord.actor
  "BikeCoordActor — the ISCO-08 7234 bicycle and related repairers
  ...
  cloud-itonami-isco-7211's foundrycoord.actor for the
  ...
  The unconditional invariant: the Bicycle Repair Coordination
  Advisor can never directly commit an operating record the
  BikeCoordGovernor refuses, and can never make this actor finalize a
  repair-execution decision (e.g. deciding to proceed with or
  complete a specific repair job) or a roadworthiness-clearance
  decision (declaring a bicycle safe to ride or sell), and can never
  override a shop safety officer's judgment — every commit-record!
  call is gated behind `:decide`, and that decision class is a
  permanent hard block regardless of disposition path. This actor
  coordinates WORKSHOP SCHEDULING/LOGISTICS ONLY — it never performs
  repair work itself or makes roadworthiness-clearance decisions
  itself."
...
(defn build-graph
  "Build a compiled BikeCoordActor graph. `store` implements
  `bikecoord.store/Store`. `advisor` implements
  `bikecoord.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:repairer-id (:repairer-id request)
                                    :op (:op proposal)
